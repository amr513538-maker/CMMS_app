...
const STATUS_OPTIONS = [
  ...
  { value: "In Progress", label: "قيد التنفيذ ⚙️" },
...
export default function RequestsPage() {
  ...
  const filtered = requests.filter(r => {
    const matchText = !searchText ||
      ...
      r.request_code?.includes(searchText) ||
      r.asset_name?.toLowerCase().includes(searchText.toLowerCase()) ||
  ...
  const getPriorityBadge = (p) => {
    switch (p) {
      ...
      case 'High': return 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400';
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">
            عرض ومتابعة كافة الطلبات المسجلة في النظام. ({filtered.length} طلب)
          </p>
        </div>
        {!isTech() && (
      ...
      {/* Filters Bar */}
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row gap-3 transition-colors">
        {/* Search */}
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="ابحث بالعنوان أو الكود أو الجهاز..."
            className="w-full pr-9 pl-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-800 dark:text-white placeholder-slate-400 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all"
          />
        </div>
        {/* Status Filter */}
        <select
          ...
          className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-800 dark:text-white focus:ring-4 focus:ring-blue-500/10 outline-none transition-all"
        >
        ...
        {(searchText || filterStatus || filterPriority) && (
          <button
            ...
            className="px-4 py-2.5 bg-red-50 dark:bg-red-900/20 text-red-500 font-bold rounded-xl text-sm hover:bg-red-100 transition-all"
          >
            مسح الفلاتر ✕
          </button>
        )}
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-bold text-lg">
              {requests.length === 0 ? "لا توجد طلبات مسجلة حالياً." : "لا توجد نتائج تطابق الفلاتر المحددة."}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-800/50 border-y border-slate-100 dark:border-slate-800">
                  ...
                  <th className="px-6 py-5 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">العنوان / الجهاز</th>
                  ...
                  {(isAdmin() || isTech()) && (
                    <th className="px-6 py-5 text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">مُقدَّم من</th>
                  )}
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {filtered.map(r => (
                  <tr key={r.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors group">
                    ...
                    {(isAdmin() || isTech()) && (
                      <td className="px-6 py-5">
                        <span className="text-slate-600 dark:text-slate-400 text-sm font-medium">{r.requested_by_name || "—"}</span>
                      </td>
                    )}
                    <td className="px-6 py-5 text-center">
                      <span className={`inline-flex px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter ${getPriorityBadge(r.priority)}`}>
                        {getPriorityLabel(r.priority)}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className={`inline-flex px-3 py-1 rounded-lg text-[11px] font-bold ${getStatusBadge(r.status)}`}>
